 ...
 */
public class ToursCrawler extends WebCrawler {
	...
	/**
	 * Instantiates a new tours crawler.
	 */
	public ToursCrawler() {
		try {
		fn = new FileWriter("c:\\tours.txt");
		out  = new BufferedWriter(fn);
	 ...
	 * 
	 * @param url the URL that should be visited.
	...
	public boolean shouldVisit(WebURL url) {
		...
		if (href.startsWith("http://www.tours.com/")) {
			return true;
		}
...
//		}
	 ...
	 * and ready to be processed by the program.
	 * 
	 * @param page the page to be analysed.
	 */
	public void visit(Page page) {
		crawlStat.incProcessedPages();
		List<WebURL> links = page.getURLs();		
		crawlStat.incTotalLinks(links.size());
		try {crawlStat.incTotalTextSize(page.getText().getBytes("UTF-8").length);} 
		catch (UnsupportedEncodingException e) {}
		if (crawlStat.getTotalProcessedPages() % 50 == 0) {dumpCrawlStat();} // We dump this crawler statistics after processing every 50 pages
...
//		extract(text);
	...
	public void dumpCrawlStat() {
		...
		System.out.println("Crawler " + myId + "> Processed Pages: " + crawlStat.getTotalProcessedPages());
		System.out.println("Crawler " + myId + "> Total Links Found: " + crawlStat.getTotalLinks());
		System.out.println("Crawler " + myId + "> Total Text Size: " + crawlStat.getTotalTextSize());
	}

	private void extract(String text) {
		try {	
//			FileWriter fn = new FileWriter("c:\\tours.txt");
//			BufferedWriter out = new BufferedWriter(fn);
			...
			while (true) {
				...
				startindex = extract(startindex, 50, "<td width='230' class='b' nowrap>", "<br>", text);
				startindex = extract(startindex, 50, "<br>", "</td>", text);
				startindex = extract(startindex, 100, "<a href='mailto:", ">", text);
		//		startindex = extract(startindex, 50, "<td class='s' nowrap>", " </td>", text);
				startindex = extract(startindex, 20, "<b>Phone:</b>", "<br>", text);
				startindex = extract(startindex, 20, "<b>Fax:</b> &nbsp; &nbsp; &nbsp; ", "</td>", text);
			}
//			out.close();
		} catch(Throwable x) {System.out.println("Error " + x.getMessage());}
	}
	
	private int extract(int startindex, int length, String start, String end, String text) throws IOException {
		startindex = text.indexOf(start, startindex) + start.length();
		int endindex = text.indexOf(end, startindex); // - end.length();
		if (startindex >= 0 && endindex >= 0 && endindex - startindex <= length ) {
			out.write("\"" + text.substring(startindex, endindex).trim() + "\",");
//			System.out.print("\"" + text.substring(startindex, endindex).trim() + "\",");
//			System.out.println("startindex " + startindex + " endindex " + endindex + " target " + text.substring(startindex, endindex));
		}
		return endindex;
	}
}
