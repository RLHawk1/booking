...
import net.cbtltd.client.widget.RazorConstants;
 ...
 */
public class AutoRazor {
//implements EntryPoint {
	
	private static final RazorConstants CONSTANTS = GWT.create(RazorConstants.class);
	private static final int wait = getParameter("wait", 15000);
	private static final int searchresults = getParameter("sr", 20);
	private static final int leadtime = getParameter("lt", 400);
	private static final int lengthofstay = getParameter("los", 10);
	private static final int lookbookratio = getParameter("lbr", 20);
	private static final int invoiceratio = getParameter("ir", 3);
	private static final int receiptratio = getParameter("rr", 2);
	 ...
	 */
	public static final Integer getParameter(String name, Integer value) {
	 ...
	 */
	private static final void lookbookUpdate(Date fromtime) {
		searchcount++;
	...
	private static long reservationtime = 0;
	 ...
	 * @param fromtime the fromtime
	 */
	private static final void reservationUpdate(Date fromtime) {
		reservationcount++;
	...
	private static long saletime = 0;
	 ...
	 * @param fromtime the fromtime
	 */
	private static final void saleUpdate(Date fromtime) {
		salecount++;
	 ...
	 * @param fromtime the fromtime
	 */
	private static final void receiptUpdate(Date fromtime) {
		receiptcount++;
	...
	private static final GuardedRequest<Session> sessionAutologin = new GuardedRequest<Session>() {
	...
	private static final GuardedRequest<Table<AvailableItem>> lookBook = new GuardedRequest<Table<AvailableItem>>() {
	...
	private static final GuardedRequest<Party> customerExists = new GuardedRequest<Party>() {
	...
	// Create Reservation
	//-----------------------------------------------
	private static final GuardedRequest<Reservation> reservationCreate = new GuardedRequest<Reservation>() {
	...
	// Update Reservation
	//-----------------------------------------------
	private static final GuardedRequest<Reservation> reservationUpdate = new GuardedRequest<Reservation>() {
		Date fromtime = new Date();
		protected boolean error() {return false;}
		protected void send() {
			fromtime = new Date();
	...
	//-----------------------------------------------
	private static GuardedRequest<ReservationEntities> reservationentitiesRead = new GuardedRequest<ReservationEntities>() {
		protected boolean error() {return false;}
	...
	//-----------------------------------------------
	private static GuardedRequest<Event<Journal>> saleCreate = new GuardedRequest<Event<Journal>>() {
		Date fromtime = new Date();
		protected boolean error() {return AbstractRoot.noOrganizationid();}
		protected void send() {
			fromtime = new Date();
	...
	//-----------------------------------------------
	private static GuardedRequest<Event<Journal>> receiptCreate = new GuardedRequest<Event<Journal>>() {
		Date fromtime = new Date();
		protected boolean error() {return AbstractRoot.noOrganizationid();}
		protected void send() {
			fromtime = new Date();
	 ...
	 */
	private static void setValue(Session session) {
...
//
//		//Uses a deferred command to catch initialization exceptions in onModuleLoad2
